...
async def post_post_process_active(payload: dict = Body(...)):
    ...
    payload: {"name": "Qwen3.5-4B-Instruct"}
    """
    name = (payload.get("name") or "").strip()
    if not name or "/" in name or "\\" in name or name in (".", ".."):
        raise HTTPException(400, f"非法模型名: {name!r}")
    ...
    if not target.is_dir():
        raise HTTPException(404, f"模型目录不存在: {name}")
    try:
        import asyncio
...
async def search_llm_models(
    ...
    try:
        result = await modelscope_search.search(
            q.strip(), page=page, page_size=page_size
        )
    except RuntimeError as exc:
        raise HTTPException(502, str(exc))
    items = [it for it in result["items"] if not it.get("is_asr")]
    downloaded_names = {m["name"] for m in models_registry.list_llm_models()}
    for it in items:
...
async def post_post_process_download(payload: dict = Body(...)):
    """从 HuggingFace 下载 GGUF 模型到 models/llm/。

    payload: {"model_id": "unsloth/Qwen3.5-4B-GGUF", "file_name": "Qwen3.5-4B-Q4_K_M.gguf"}
    """
    model_id = (payload.get("model_id") or "").strip()
    ...
    if not file_name:
        raise HTTPException(400, "缺少 file_name")

    import asyncio

    target = config.LLM_MODELS_DIR / file_name
    if target.exists():
        return {"ok": True, "name": file_name, "cached": True}

    logger.info("GGUF 下载: %s / %s → models/llm/%s", model_id, file_name, file_name)
    ...
    def _run():
        ...
        hf_hub_download(
            repo_id=model_id,
            filename=file_name,
            local_dir=str(config.LLM_MODELS_DIR),
        )

    try:
        await asyncio.to_thread(_run)
    except Exception as exc:
        logger.exception("GGUF 下载失败: %s", model_id)
...
async def delete_post_process_model(name: str):
    ...
    target = config.LLM_MODELS_DIR / name
    try:
        target.resolve().relative_to(config.LLM_MODELS_DIR.resolve())
    except ValueError:
        raise HTTPException(400, "目标路径不在 models/llm/ 目录下")
    if not target.is_dir():
        raise HTTPException(404, f"模型不存在: {name}")
    if target.is_symlink():
        target.unlink()
        ...
        shutil.rmtree(target)
    logger.info("已删除 LLM 模型目录: %s", target)
    return {"ok": True, "deleted": name}
